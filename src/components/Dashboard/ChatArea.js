...
export const ChatArea = () => {
  ...
  const [messagesn, setMessages] = useState([]);
  const [newMessage, setNewMessage] = useState("");
  useEffect(() => {
    const socket = io(); // Make sure your server is emitting 'newMessage' events

    // Listen for new message event
    socket.on('newMessage', (message) => {
      setMessages((prevMessages) => [...prevMessages, messagesn]);
      ...
      {context.activeChat !== "" ? (
        ...
        !context.ischatLoading && (
          ...
          >
            ...
            <Text fontSize={"2vw"}>Online Ludo app</Text>
            <Text fontSize={"md"}>Select a chat to start messaging and play Ludo</Text>
          </Box>
        )
      )}
    </>
